...
function verifyClassName(name) {
    if (typeof name == 'string' && name.length > 0) {
        if (RegularExpression_1.default.CLASS_NAME.test(name))
            ...
            throw new OtherErrors_1.FormattedError(ExceptionMessages_1.default.CLASS_NAME_CONTAIN);
        else
            throw new OtherErrors_1.FormattedError(ExceptionMessages_1.default.START_CLASS_NAME);
    }
    else
        throw new OtherErrors_1.FormattedError(ExceptionMessages_1.default.SPECIFY_CLASS_NAME);
}
...
function verifyFunctionName(name) {
    if (typeof name == 'string' && name.length > 0) {
        if (RegularExpression_1.default.FUNCTION_NAME.test(name))
            ...
            throw new OtherErrors_1.FormattedError(ExceptionMessages_1.default.FUNCTION_NAME_CONTAIN);
        else
            throw new OtherErrors_1.FormattedError(ExceptionMessages_1.default.START_FUNCTION_NAME);
    }
    else
        throw new OtherErrors_1.FormattedError(ExceptionMessages_1.default.SPECIFY_FUNCTION_NAME);
}
