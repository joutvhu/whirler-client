...
export function verifyClassName(name) {
    if (typeof name === 'string' && name.length > 0) {
        if (RegularExpression.CLASS_NAME.test(name))
            ...
            throw new FormattedError(ExceptionMessages.START_CLASS_NAME);
    }
    else
        throw new FormattedError(ExceptionMessages.SPECIFY_CLASS_NAME);
}
export function verifyFunctionName(name) {
    if (typeof name === 'string' && name.length > 0) {
        if (RegularExpression.FUNCTION_NAME.test(name))
            ...
            throw new FormattedError(ExceptionMessages.START_FUNCTION_NAME);
    }
    else
        throw new FormattedError(ExceptionMessages.SPECIFY_FUNCTION_NAME);
}
